...
.arrow-link {
    color: #9b2035; /* Example link color */
    text-decoration: none;
        ...
        .then(data => {
            ...
            if (data.feedcontent && data.feedcontent.length) {
                data.feedcontent.forEach(title => {
                    ...
                    div.classList.add('title', 'rounded-container'); // Add classes for styling
                    div.innerHTML = `
                        <h3>${title.Title}</h3>
                        <p>Section: ${title.Captions}</p>   
                        <div class="button-group">
                            <button onclick="editGrade(${title.ContentID}, '${title.Title}', '${title.Captions}')">Edit</button>
                            <button class="btn-small" onclick="deleteGrade(${title.ContentID})">Delete</button>
                            <a href="content.php?ContentID=${title.ContentID}&Title=${encodeURIComponent(title.Title)}&Captions=${encodeURIComponent(title.Captions)}" class="arrow-link">
